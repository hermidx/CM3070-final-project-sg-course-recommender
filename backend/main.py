...
# ✅ Chat helpers
# -------------------------
def detect_chat_filters(message: str) -> Dict[str, Any]:
    ...
    if any(x in lower for x in ["polytechnic", "poly"]):
        institution_type = "Poly"
    elif any(x in lower for x in ["university", "uni"]):
        institution_type = "University"

    if "diploma" in lower:
        level = "Diploma"
    elif any(x in lower for x in ["bachelor", "degree", "undergraduate"]):
        level = "Bachelor's"
    elif any(x in lower for x in ["master", "masters", "postgraduate"]):
        level = "Master's"

    institutions = sorted(
        [x for x in df["institution"].dropna().astype(str).unique() if str(x).strip()],
        key=len,
        reverse=True,
    )
    for inst_name in institutions:
        if inst_name.lower() in lower:
            institution = inst_name
            break

    requires_math = bool(re.search(r"\b(math|mathematics)\b", lower))
    requires_english = bool(re.search(r"\benglish\b", lower))

    cleaned = raw
    noise_patterns = [
        r"\b(i want|i am looking for|looking for|find me|show me|recommend|suggest|course|courses|school|schools|suitable|best|please|can you|help me|i like|i am interested in|interested in)\b",
        r"\b(polytechnic|poly|university|uni|diploma|bachelor's|bachelor|degree|undergraduate|master's|masters|master|postgraduate)\b",
        r"\b(math|mathematics|english)\b",
    ]
    for pat in noise_patterns:
        cleaned = re.sub(pat, " ", cleaned, flags=re.IGNORECASE)

    if institution != "All":
        cleaned = re.sub(re.escape(institution), " ", cleaned, flags=re.IGNORECASE)
...
def build_chat_reply(parsed: Dict[str, Any], results: List[Dict[str, Any]]) -> str:
    parts = []
    if parsed.get("level") and parsed["level"] != "All":
        parts.append(parsed["level"])
    if parsed.get("institution_type") and parsed["institution_type"] != "All":
        parts.append(parsed["institution_type"])
    if parsed.get("institution") and parsed["institution"] != "All":
        parts.append(parsed["institution"])

    target = " / ".join(parts) if parts else "your preferences"

    if not results:
        return (
            f"I couldn’t find a close match for {target}. "
            "Try asking with a clearer interest like business, cybersecurity, AI, design, data analytics, engineering."
        )

    top = results[0]
    return (
        f"I found {len(results)} option(s) matching {target}. "
        f"Top pick: {top['course_name']} at {top['institution']}. "
        "I’ve shown the matching course cards below — you can compare or save them."
    )
...
def meta(
    ...
    def is_all(v: Optional[str]) -> bool:
        return v is None or str(v).strip() == "" or str(v).strip().lower() == "all"

    scoped = df.copy()

    if not is_all(institution_type):
        scoped = scoped[scoped["institution_type"] == institution_type]

    levels_df = scoped.copy()
    ...
    return {
        "institution_types": sorted(
            [x for x in df["institution_type"].dropna().astype(str).unique() if x.strip()]
        ),
        "levels": sorted(
            [x for x in levels_df["level"].dropna().astype(str).unique() if x.strip()]
        ),
        "institutions": sorted(
            [x for x in scoped["institution"].dropna().astype(str).unique() if x.strip()]
        ),
    }
...
def chat(req: ChatRequest):
    try:
        if not req.message or not req.message.strip():
            return {
                "reply": "Hi, I’m Harneet — your SG Study Guide. Ask me e.g. “diploma business poly”, “cybersecurity degree”, “NUS courses”.",
                "results": [],
                "applied_filters": {},
                "note": "Chat suggestions are grounded in the dataset.",
                "ui_meta": {
                    "query": "",
                    "result_count": 0,
                    "mode": "chat",
        ...
        parsed = detect_chat_filters(req.message)

        top_k = max(1, min(int(req.top_k), TOP_K_MAX))
        use_sem = bool(SEMANTIC_AVAILABLE)
        ...
        search_req = SearchRequest(
            ...
            hybrid=use_sem,
            requires_math=parsed["requires_math"],
            requires_english=parsed["requires_english"],
        )

        search_out = search(search_req)
        results = search_out.get("results", []) or []

        reply = build_chat_reply(parsed, results)

        ui_meta = search_out.get("ui_meta", {}) or {}
        ui_meta["mode"] = "chat"

        note = search_out.get("note", "Chat suggestions are grounded in the dataset.")

        return {
            "reply": reply,
            "results": results,
            "applied_filters": parsed,
            "note": note,
            "ui_meta": ui_meta,
        }
