...
const elChatWidget = document.getElementById("chatWidget");
...
const elChatBtn = document.getElementById("chatBtn");
...
// --------------------
// ✅ Harneet chatbot helpers
// --------------------
function addChatBubble(role, htmlOrText) {
  if (!elChatMessages) return;

  const div = document.createElement("div");
  div.className = `chatBubble ${role}`;

  if (role === "bot" && String(htmlOrText).includes("<")) {
    div.innerHTML = String(htmlOrText);
  } else {
    div.textContent = String(htmlOrText);
  }
...
async function doChatSearch() {
  ...
  if (elChatBtn) elChatBtn.disabled = true;
  ...
  try {
    ...
    if (data.applied_filters) {
      if (elType && data.applied_filters.institution_type) {
        elType.value = data.applied_filters.institution_type;
      }

      await loadMeta(true);

      if (elLevel && data.applied_filters.level) {
        elLevel.value = data.applied_filters.level;
      }

      await loadMeta(true);

      if (elInst && data.applied_filters.institution) {
        elInst.value = data.applied_filters.institution;
      }

      if (elRequiresMath)
        elRequiresMath.checked = !!data.applied_filters.requires_math;
      if (elRequiresEnglish)
        elRequiresEnglish.checked = !!data.applied_filters.requires_english;

      if (elUseSemantic) elUseSemantic.checked = true;
      if (elUseHybrid) {
        elUseHybrid.disabled = false;
        elUseHybrid.checked = true;
      }
    }

    if (data.results && data.results.length) {
      showSearchView();
      lastQueryText = message;
      setMsg("");
      renderResults(data.results, data.ui_meta || null, data.note || "");
    } else {
      setMsg(data.message || "No matches. Try different keywords.");
    ...
    addChatBubble(
      "bot",
      "I couldn’t reach the backend. Make sure FastAPI is running."
    );
  } finally {
    if (elChatBtn) elChatBtn.disabled = false;
  }
}
...
// Load meta
// --------------------
async function loadMeta(preserveSelection = true) {
  try {
    const currentType = elType ? elType.value : "All";
    const currentLevel = elLevel ? elLevel.value : "All";
    const currentInst = elInst ? elInst.value : "All";

    const params = new URLSearchParams({
      institution_type: currentType || "All",
      level: currentLevel || "All",
    });

    const res = await fetch(`${API}/api/meta?${params.toString()}`, {
      mode: "cors",
    });
    ...
    if (preserveSelection) {
      if (elType) {
        elType.value = meta.institution_types.includes(currentType)
          ? currentType
          : "All";
      }

      if (elLevel) {
        elLevel.value = meta.levels.includes(currentLevel)
          ? currentLevel
          : "All";
      }

      if (elInst) {
        elInst.value = meta.institutions.includes(currentInst)
          ? currentInst
          : "All";
      }
    }
...
async function refreshFiltersFromSelection(changed) {
  const prevType = elType ? elType.value : "All";
  const prevLevel = elLevel ? elLevel.value : "All";
  const prevInst = elInst ? elInst.value : "All";

  if (changed === "type") {
    if (elLevel) elLevel.value = "All";
    if (elInst) elInst.value = "All";
  }
  ...
  if (changed === "type" && elType) {
    elType.value = prevType;
    await loadMeta(true);
  }

  if (changed === "level") {
    if (elType) elType.value = prevType;
    await loadMeta(true);
    if (elLevel) elLevel.value = prevLevel;
    await loadMeta(true);
  }
  ...
  if (changed === "institution") {
    ...
    if (elLevel) elLevel.value = prevLevel;
    await loadMeta(true);
    if (elInst) elInst.value = prevInst;
  }
}
...
if (elType) {
  elType.addEventListener("change", async () => {
    await refreshFiltersFromSelection("type");
  });
}

if (elLevel) {
  elLevel.addEventListener("change", async () => {
    await refreshFiltersFromSelection("level");
  });
}

if (elInst) {
  elInst.addEventListener("change", async () => {
    await refreshFiltersFromSelection("institution");
  });
}
...
// --------------------
// ✅ Harneet chatbot open/close + send
// --------------------
if (elChatToggle && elChatWidget) {
  elChatToggle.addEventListener("click", () => {
    elChatWidget.classList.toggle("hidden");
    if (!elChatWidget.classList.contains("hidden") && elChatInput) {
      elChatInput.focus();
    }
  });
}

if (elChatClose && elChatWidget) {
  elChatClose.addEventListener("click", () => {
    elChatWidget.classList.add("hidden");
  });
}

if (elChatBtn) elChatBtn.addEventListener("click", doChatSearch);

if (elChatInput) {
  elChatInput.addEventListener("keydown", (e) => {
    if (e.key === "Enter") doChatSearch();
  });
}
